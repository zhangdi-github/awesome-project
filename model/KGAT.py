...
class Aggregator(nn.Module):
    """KGAT的聚合器， gcn, graphsage, bi-interaction"""
...
class KGAT(nn.Module):
    ...
    def __init__(self, args,
        ...
        self.n_users = n_users  # 用户数、实体数、关系数
        self.n_entities = n_entities
        self.n_relations = n_relations

        self.embed_dim = args.embed_dim  # 实体嵌入维度，包括user、item和知识图中其他entity
        self.relation_dim = args.relation_dim  # 关系嵌入维度

        self.aggregation_type = args.aggregation_type  # 消息聚合类型
        self.conv_dim_list = [args.embed_dim] + eval(args.conv_dim_list)  # 多层GCN的每一层维度
        self.mess_dropout = eval(args.mess_dropout)  # 每一层的图卷积hidden_size
        self.n_layers = len(eval(args.conv_dim_list))  # 图卷积的层数, 默认参数为3

        self.kg_l2loss_lambda = args.kg_l2loss_lambda  # TranR训练的L2损失系数
        self.cf_l2loss_lambda = args.cf_l2loss_lambda  # 协同过滤训练的L2损失系数

        # 定义embedding矩阵（也就是lookup table）
        # 实体&用户的embedding，实体在前，user在后
        # 即：实体&用户embedding = concat(item emb， 知识图除了item的其他entity emb，user emb)
    ...
    def calc_cf_embeddings(self):
        ...
        # Equation (11) ----layer-aggregation mechanism
